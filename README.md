...
1. **把 `.txt` 请求转发到本服务的 3000 端口**，且**不重写路径**——微信校验文件必须位于域名根路径；本服务同时支持子目录路径（如 `/h5/xxx.txt`），一条规则转发任意深度的 `.txt` 即可。
...
**完整配置示例见「网关接入」分组文档：** [Nginx](docs/网关接入/nginx.md)、[Traefik](docs/网关接入/traefik.md)、[Caddy](docs/网关接入/caddy.md)、[Apache / httpd](docs/网关接入/apache.md)、[Kong / APISIX](docs/网关接入/kong-apisix.md)。管理端登录后侧边导航点「帮助」也能直接查看——`docs/` 目录里的每篇 md 都会自动出现在帮助面板里，增加文档只需放文件。
...
**子目录路径**：本服务定位是文本路由器，路径字段可含子目录（如 `h5/xxx.txt`，任意深度），每段 1-80 位字母、数字、下划线、连字符或点，总长不超过 255。注意微信校验文件本身仍必须位于域名根路径，子目录只用于站内其他文本路由场景。

**帮助**：侧边导航「帮助」tab 打开帮助面板，列出 `docs/` 目录里的所有 md 文档（标题取自文件首行 `#`；`docs/` 下的子目录自动作为分组显示，列表顶部支持按标题搜索过滤），左侧选择、右侧渲染，新增文档只需放文件即可，服务无需重启。
...
- **每日定时备份**：进程内置定时器（默认关闭；开启后每天 23:00 执行，默认保留 7 份），不依赖外部 cron
...
测试已在 **Node 22.21.1**（与 Dockerfile 同大版本）上全量跑通：18 个测试文件、**320 项全过**。
